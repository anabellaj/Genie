...
class CarouselFlashcard extends StatefulWidget{
  ...
  final Groups group;
  const CarouselFlashcard({super.key, required this.flashcards, required this.topicId, required this.group});
...
class _CarouselFlashcard extends State<CarouselFlashcard>{
  ...
  void studiedFlashcard()async{
    await ControllerStudy.updateStudied(studied, widget.topicId, widget.flashcards);
    ScaffoldMessenger.of(context).hideCurrentMaterialBanner();
    Navigator.pushReplacement(context, 
      MaterialPageRoute(builder: (context)=> TopicScreen(topicId: widget.topicId, group: widget.group,))
    );  
  }
 ...
 void getInfo()async{
  ...
  Widget build(BuildContext context) {
    ...
    Column(
      ...
      Center(child: Text('Todavía no hay fichas disponibles'),):
      Column(
        children: [
        CarouselSlider(
        ...
        , carouselController: _controller,
          ...
          children: [
            ...
            }, icon: Icon(Icons.chevron_left, color: genieThemeDataDemo.colorScheme.primary)),

            Row(children: widget.flashcards.map((f){
              return GestureDetector(
                onTap: () => _controller.animateToPage(widget.flashcards.indexOf(f)),
              child: Container(
                width: 6.0,
                height: 6.0,
                ...
                decoration: BoxDecoration(
                    ...
                    color: genieThemeDataDemo.colorScheme.primary.withOpacity(_current == widget.flashcards.indexOf(f) ? 0.9 : 0.4)),
              ),
              );
            }).toList(),),
            IconButton(onPressed: (){
              ...
              _controller.animateToPage(_current);
            }, icon: Icon(Icons.chevron_right, color: genieThemeDataDemo.colorScheme.primary,)),
